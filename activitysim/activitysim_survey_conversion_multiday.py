...
person_weight = 'person_adult_weight_2021'   # no specific person weight available, use HH weight
...
if geocode_trips:
    ...
    if read_survey_from_db:
        trip = pd.read_sql(sql='SELECT * FROM HHSurvey.v_trips WHERE survey_year = 2021', con=engine)
    else:
...
if geocode_households:
    ...
    if read_survey_from_db:
        hh = pd.read_sql(sql='SELECT * FROM HHSurvey.v_households WHERE survey_year = 2021', con=engine)  
    else:
...
if geocode_persons:
    ...
    if read_survey_from_db:
       person = pd.read_sql(sql='SELECT * FROM HHSurvey.v_persons WHERE survey_year = 2021', con=engine)
    else:
