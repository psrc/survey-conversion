...
geocode_households = True
geocode_trips = True
geocode_persons = True

# Raw survey files can be read locally instead of from PSRC database
read_survey_from_db = True
...
output_dir = r"data\survey_data_2021"
geolocated_output = r"data\survey_conversion_2021"

parcel_file = r"R:\e2projects_two\activitysim\survey\input_data\survey_input_large_files\parcels_urbansim.txt"
...
# Weight
hh_weight = "hh_weight_2021"
person_weight = (
    "person_adult_weight_2021"  # no specific person weight available, use HH weight
)
trip_weight = "trip_adult_weight_2021"
tour_weight = "trip_adult_weight_2021"  # FIXME: this needs work | average the trip_weight_2017_2019 across the tour
...
def edit_school_location(
    person, gdf_lu, school_segment, enrollment_col, student_type_col
):
    person = person.merge(
        gdf_lu[["MAZ", "geometry", enrollment_col]],
        left_on="school_zone_id",
        right_on="MAZ",
        how="left",
    )

    person.loc[
        (person["school_zone_id"] > 0)
        & (person[enrollment_col] == 0)
        & (person[student_type_col] == 1),
        "edit_school_loc",
    ] = 1
    ...
    nearest_df = find_nearest(
        df[["person_id", "school_zone_id", "geometry"]],
        school_maz_gdf[["MAZ", "geometry"]],
    )
    nearest_df.rename(columns={"MAZ": "updated_school_maz"}, inplace=True)
    nearest_df[["person_id", "school_zone_id", "updated_school_maz", "dist"]].to_csv(
        os.path.join(output_dir, "temp", school_segment + "_location_edit.csv")
    )

    person = person.merge(
        nearest_df[["person_id", "updated_school_maz"]], on="person_id", how="left"
    )
    person["updated_school_maz"] = person["updated_school_maz"].fillna(-1).astype("int")
    person.loc[person["edit_school_loc"] == 1, "school_zone_id"] = person[
        "updated_school_maz"
    ]
    person.drop(
        ["MAZ", "geometry", enrollment_col, "updated_school_maz", "edit_school_loc"],
        inplace=True,
        axis=1,
    )
...
if geocode_households or geocode_trips or geocode_persons:
    ...
    parcels_gdf = parcels_gdf.merge(
        parcel_maz, left_on="parcelid", right_on="parcel_id", how="left"
    )
...
if geocode_trips:
    if read_survey_from_db:
        trip = pd.read_sql(
            sql="SELECT * FROM HHSurvey.v_trips WHERE survey_year = 2021", con=engine
        )
    else:
    ...
    # Origins
    ####################
    trip_o_gdf = gpd.GeoDataFrame(
        trip[["trip_id", "origin_lat", "origin_lng"]],
        geometry=gpd.points_from_xy(trip.origin_lng, trip.origin_lat),
    )
    ...
    nearest_df = find_nearest(
        trip_o_gdf, parcels_gdf[["parcel_id", "maz_id", "geometry"]]
    )
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "trip_o_maz_location.csv"), index=False
    )
    ...
    # Destinations
    ####################
    trip_d_gdf = gpd.GeoDataFrame(
        trip[["trip_id", "dest_lat", "dest_lng"]],
        geometry=gpd.points_from_xy(trip.dest_lng, trip.dest_lat),
    )
    ...
    logger.info(
        f"Dropped {prev_len - len(trip_d_gdf)} trips: destinations outside region"
    )

    # Snap to nearest parcel
    nearest_df = find_nearest(
        trip_d_gdf, parcels_gdf[["parcel_id", "maz_id", "geometry"]]
    )
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "trip_d_maz_location.csv"), index=False
    )
...
if geocode_households:
    if read_survey_from_db:
        hh = pd.read_sql(
            sql="SELECT * FROM HHSurvey.v_households WHERE survey_year = 2021",
            con=engine,
        )
    else:
    ...
    hh_gdf = gpd.GeoDataFrame(
        hh[["household_id", "final_home_lat", "final_home_lng"]],
        geometry=gpd.points_from_xy(hh.final_home_lng, hh.final_home_lat),
    )
    ...
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "household_maz_location.csv"), index=False
    )
...
if geocode_persons:
    ...
    if read_survey_from_db:
        person = pd.read_sql(
            sql="SELECT * FROM HHSurvey.v_persons WHERE survey_year = 2021", con=engine
        )
    else:
    ...
    person[["person_id_str", "household_id_str"]] = person[
        ["person_id", "household_id"]
    ].astype("str")
    person["PNUM"] = person.apply(
        lambda x: x["person_id_str"].replace(x["household_id_str"], "").strip(), axis=1
    ).astype("int")
    ...
    person.loc[
        person["person_id"].isin(school_trip_makers) & (person.age <= 18), "pstudent"
    ] = PSTUDENT_GRADE_OR_HIGH
    person.loc[
        person["person_id"].isin(school_trip_makers) & (person.age > 18), "pstudent"
    ] = PSTUDENT_UNIVERSITY
    ...
    person.loc[
        (person.pstudent == PSTUDENT_GRADE_OR_HIGH)
        & (person.age <= GRADE_SCHOOL_MAX_AGE),
        "is_gradeschool",
    ] = 1
    person.loc[
        (person.pstudent == PSTUDENT_GRADE_OR_HIGH)
    ...
    person.loc[
        (person["is_gradeschool"] == 1)
        | (person["is_highschool"] == 1)
        | (person["is_university"] == 1),
        "is_student",
    ] = 1
    person.loc[
        (person["is_student"] == 1) & (person["school_loc_lat"].isnull()),
        "student_missing_school_loc",
    ] = 1
    person_list = person[person["student_missing_school_loc"] == 1].person_id
    df = trip.loc[
        (trip["person_id"].isin(person_list))
        & (
            trip["dest_purpose"]
            == "Went to school/daycare (e.g., daycare, K-12, college)"
        )
    ...
    df_imputed["imputed_school_loc_lat"] = df.groupby("person_id")["dest_lat"].agg(
        lambda x: pd.Series.mode(x)[0]
    )
    df_imputed["imputed_school_loc_lng"] = df.groupby("person_id")["dest_lng"].agg(
        lambda x: pd.Series.mode(x)[0]
    )
    ...
    logger.info(
        f'Dropped {len(person[person["student_missing_school_loc"] == 1])} persons for students without school location information'
    )
    person = person[person["student_missing_school_loc"] != 1]

    _filter = person["school_loc_lat"] > 0
    person_gdf = gpd.GeoDataFrame(
        person[_filter][
            [
                "person_id",
                "school_loc_lat",
        ...
        geometry=gpd.points_from_xy(
            person[_filter].school_loc_lng, person[_filter].school_loc_lat
        ),
    )
    ...
    nearest_df = find_nearest(
        gdf_gradeschool, gradeschool_parcels[["parcel_id", "maz_id", "geometry"]]
    )
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "gradeschool_maz_location.csv"), index=False
    )
    person = person.merge(
        nearest_df[["person_id", "maz_id"]], on="person_id", how="left"
    )
    ...
    nearest_df = find_nearest(
        gdf_highschool, highschool_parcels[["parcel_id", "maz_id", "geometry"]]
    )
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "highschool_maz_location.csv"), index=False
    )
    person = person.merge(
        nearest_df[["person_id", "maz_id"]], on="person_id", how="left"
    )
    ...
    nearest_df = find_nearest(
        gdf_university, university_parcels[["parcel_id", "maz_id", "geometry"]]
    )
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "university_maz_location.csv"), index=False
    )
    person = person.merge(
        nearest_df[["person_id", "maz_id"]], on="person_id", how="left"
    )
    ...
    person.loc[
        (person["pemploy"].isin([1, 2])) & (person["work_lat"].isnull()),
        "worker_missing_work_loc",
    ] = 1
    worker_list = person[person["worker_missing_work_loc"] == 1].person_id
    df = trip.loc[
        (
            trip["dest_purpose"].isin(
                [
    ...
    df_imputed["imputed_work_lat"] = df.groupby("person_id")["dest_lat"].agg(
        lambda x: pd.Series.mode(x)[0]
    )
    df_imputed["imputed_work_lng"] = df.groupby("person_id")["dest_lng"].agg(
        lambda x: pd.Series.mode(x)[0]
    )
    ...
    person_gdf = gpd.GeoDataFrame(
        person[_filter][["person_id", "work_lat", "work_lng"]],
    ...
    nearest_df = find_nearest(
        person_gdf, work_parcels[["parcel_id", "maz_id", "geometry"]]
    )
    nearest_df.to_csv(
        os.path.join(output_dir, "temp", "work_location.csv"), index=False
    )
    person = person.merge(
        nearest_df[["person_id", "maz_id"]], on="person_id", how="left"
    )
    ...
    person.loc[
        person["pemploy"].isin([1, 2]) & (person["workplace_zone_id"].isnull()),
        "pemploy",
    ] = 3
    ...
    person.to_csv(
        os.path.join(geolocated_output, "elmer_person_geocoded.csv"), index=False
    )
...
if convert_survey:
    ...
    for person_type in person[person_type_field].unique():
        ...
        hh[person_type_dict[person_type]] = hh[person_type_dict[person_type]].astype(
            "int"
        )
    ...
    trip = trip.merge(
        person[[person_id_col, school_taz, work_taz]], how="left", on=person_id_col
    )
    ...
    logger.info(
        f"Dropped {len(trip[filter])} trips with same origin and destination MAZ"
    )
    trip = trip[~filter]
    ...
    for day in day_list:
        trip.loc[trip["dayofweek"] == day, "household_id"] = (
            trip["original_household_id"].astype("str") + "_" + str(day_dict[day])
        )

        hh_day = hh[
            hh["household_id"].isin(
                trip.loc[trip["dayofweek"] == day, "original_household_id"]
            )
        ].copy()
        hh_day["household_id"] = (
            hh_day["original_household_id"].astype("str") + "_" + str(day_dict[day])
        )
        hh_day["flag"] = 1
        hh = hh.append(hh_day)

        person_day = person_copy[
            person_copy["original_person_id"].isin(
                trip[trip["dayofweek"] == day]["person_id"]
            )
        ]
        person_day["household_id"] = (
            person_day["original_household_id"].astype("str") + "_" + str(day_dict[day])
        )
        person_day["person_id"] = (
            person_day["original_person_id"].astype("str") + "_" + str(day_dict[day])
        )
        person_day["flag"] = 1
        person = person.append(person_day)

        trip.loc[trip["dayofweek"] == day, "person_id"] = (
            trip["original_person_id"].astype("str") + "_" + str(day_dict[day])
        )
    ...
    logger.info(
        f"Dropped {len(trip[~trip['person_id'].isin(person['person_id'])])} trips because person was removed from data"
    )
    ...
    if len(trip[filter]) > 0:
        bad_trips = flag_trips(
            trip[filter],
            bad_trips,
            "trips have the same origin and destination of home",
        )
        trip = trip[~filter]
    ...
    if len(trip[filter]) > 0:
        bad_trips = flag_trips(
            trip[filter], bad_trips, "missing trip destination purpose"
        )
        trip = trip[~filter]
    ...
    for personid in trip[person_id_col].unique():
        ...
        if (df.groupby(person_id_col).first()[opurp].values[0] != "Home") or df.groupby(
            person_id_col
        ).last()[dpurp].values[0] != "Home":
            #    # Flag this set
            for i in df[trip_id].to_list():
        ...
        for tour_start_index in range(len(home_tours_start)):
            ...
            tour_dict[tour_id][toadtyp] = _df.iloc[0][
                oadtyp
            ]  # Tour origin address type

            # Last row contains return information
            tour_dict[tour_id]["tarorig"] = _df.iloc[-1][
                arrtm
            ]  # Tour arrive time at origin (return time)
            ...
            if len(_df) == 2:
                ...
                tour_dict[tour_id][tour_mode] = assign_tour_mode(
            ...
            # For tour groups with > 2 trips, calculate primary purpose and halves; first deal with subtours
                ...
                if (
                    (len(_df) >= 4)
                    & (len(_df[_df[oadtyp] == adtyp_work]) >= 2)
                    & (len(_df[_df[opurp] == purp_work]) >= 2)
                    & (
                        len(
                            _df[
                                (_df[oadtyp] == adtyp_work)
                                & (~_df[dadtyp].isin([purp_work, purp_home]))
                            ]
                        )
                        >= 1
                    )
                    ...
                    subtour_index_start_values = _df[
                        (
                            ...
                            | (
                                (_df[opurp] == purp_work)
                                & (~_df[dpurp].isin([purp_work, purp_home]))
                            )
                        )
                    ].index.values
                    ...
                    for subtour_start_value in subtour_index_start_values:
                        ...
                        for i in _df.index.values[next_row_index_start:]:
                            next_row = _df.loc[i]
                            if (
                                next_row[dadtyp] == adtyp_work
                            ):  # Assuming we only have work-based subtours
                                tour_id += 1
                                ...
                                tour_dict[tour_id][person_id_col] = subtour_df.iloc[0][
                                    person_id_col
                                ]
                                tour_dict[tour_id]["day"] = day
                                tour_dict[tour_id]["tlvorig"] = subtour_df.iloc[0][
                                    deptm
                                ]
                                tour_dict[tour_id]["tarorig"] = subtour_df.iloc[-1][
                                    arrtm
                                ]
                                ...
                                tour_dict[tour_id][
                                    parent
                                ] = parent_tour_id  # Parent is the main tour ID
                                tour_dict[tour_id][
                                    "subtrs"
                                ...
                                trip.loc[
                                    trip[trip_id].isin(subtour_df[trip_id].values),
                                    "tour",
                                ] = tour_id

                                if len(subtour_df) == 2:
                                    tour_dict[tour_id][tdpurp] = subtour_df.iloc[0][
                                        "purpose"
                                    ]
                                    tour_dict[tour_id]["tripsh1"] = 1
                                    tour_dict[tour_id]["tripsh2"] = 1
                                    tour_dict[tour_id][tdadtyp] = subtour_df.iloc[0][
                                        dadtyp
                                    ]
                                    tour_dict[tour_id][toadtyp] = subtour_df.iloc[0][
                                        oadtyp
                                    ]
                                    tour_dict[tour_id][tdtaz] = subtour_df.iloc[0][dtaz]
                                    tour_dict[tour_id][tdpcl] = subtour_df.iloc[0][dpcl]
                                    tour_dict[tour_id]["tlvdest"] = subtour_df.iloc[-1][
                                        deptm
                                    ]
                                    tour_dict[tour_id]["tardest"] = subtour_df.iloc[0][
                                        arrtm
                                    ]
                                    tour_dict[tour_id][tour_mode] = assign_tour_mode(
                                        subtour_df, tour_dict, tour_id
                                    )

                                    # Set tour half and tseg within half tour for trips
                                    # for tour with only two records, there will always be two halves with tseg=1 for both
                                    trip.loc[
                                        trip[trip_id] == subtour_df.iloc[0][trip_id],
                                        "half",
                                    ] = 1
                                    trip.loc[
                                        trip[trip_id] == subtour_df.iloc[-1][trip_id],
                                        "half",
                                    ] = 2
                                    trip.loc[
                                        trip[trip_id].isin(_df[trip_id]), "tseg"
                                    ] = 1
                                ...
                                # If subtour length > 2, find the primary purpose/destination
                                    ...
                                    primary_subtour_purp_index = subtour_df[
                                        subtour_df[dpurp] != "Change mode"
                                    ]["duration"].idxmax()
                                    tour_dict[tour_id][tdpurp] = subtour_df.loc[
                                    ...
                                    tour_dict[tour_id][tdtaz] = subtour_df.loc[
                                        primary_subtour_purp_index
                                    ][dtaz]
                                    tour_dict[tour_id][tdpcl] = subtour_df.loc[
                                        primary_subtour_purp_index
                                    ][dpcl]
                                    tour_dict[tour_id][tdadtyp] = subtour_df.loc[
                                        primary_subtour_purp_index
                                    ][dadtyp]
                                    ...
                                    tour_dict[tour_id]["tripsh2"] = len(
                                        subtour_df.loc[primary_subtour_purp_index + 1 :]
                                    )
                                    ...
                                    tour_dict[tour_id]["tlvdest"] = subtour_df.loc[
                                        primary_subtour_purp_index
                                    ][deptm]
                                    tour_dict[tour_id]["tardest"] = subtour_df.loc[
                                        primary_subtour_purp_index
                                    ][arrtm]
                    ...
                    if len(subtours_df) < 1:
                        ...
                        tour_dict[tour_id][tdpurp] = _df.loc[primary_purp_index][
                            "purpose"
                        ]
                        ...
                        tour_dict[parent_tour_id][tdtaz] = _df.loc[
                            main_tour_start_index
                        ][dtaz]
                        tour_dict[parent_tour_id][tdpcl] = _df.loc[
                            main_tour_start_index
                        ][dpcl]
                        tour_dict[parent_tour_id][tdadtyp] = _df.loc[
                            main_tour_start_index
                        ][dadtyp]
                        ...
                        tour_dict[parent_tour_id]["tripsh1"] = len(
                            _df.loc[0:main_tour_start_index]
                        )
                        tour_dict[parent_tour_id]["tripsh2"] = len(
                        ...
                        # Departure/arrival times
                        tour_dict[parent_tour_id]["tlvdest"] = _df.loc[
                            main_tour_end_index
                        ][deptm]
                        tour_dict[parent_tour_id]["tardest"] = _df.loc[
                            main_tour_start_index
                        ][arrtm]
                        ...
                        # Mode
                        tour_dict[parent_tour_id][tour_mode] = assign_tour_mode(
                            _df, tour_dict, tour_id
                        )
                        ...
                        trip.loc[
                            trip[trip_id].isin(df_unique_no_subtours[trip_id].values),
                            "tour",
                        ] = parent_tour_id
                    ...
                    trip.loc[
                        trip[trip_id].isin(_df.loc[primary_purp_index + 1 :].trip_id),
                        "half",
                    ] = 2
    ...
    tour.loc[
        (tour["tour_type"].isin(["work", "school"]))
    ...
    tour[~filter].to_csv(
        os.path.join(output_dir, "temp", "tours_removed_non_canoncial.csv")
    )
    tour = tour[filter]
    ...
    for index, row in tour.iterrows():
        if row.tour_id not in skip_tour:
            ...
            filter = (
                (tour.day == row.day)
                & (tour.origin == row.origin)
                & (tour.destination == row.destination)
                & (tour.start == row.start)
                & (tour.end == row.end)
                & (tour.household_id == row.household_id)
            )
    ...
    test_list = []
    for tour_id in tour.loc[
        (tour["joint_tour"].isnull())
        & (~tour["tour_type"].isin(["work", "school", "escort"])),
        "tour_id",
    ]:
        print(tour_id)
    ...
    for joint_tour in joint_tour_list:
        df.loc[df["joint_tour"] == joint_tour, "tour_id"] = df[
            df["joint_tour"] == joint_tour
        ].iloc[0]["tour_id"]
    ...
    df["participant_id"] = df["tour_id"].astype("str") + df["participant_num"].astype(
        "int"
    ).astype("str")

    df = df[
        ["person_id", "tour_id", "household_id", "participant_num", "participant_id"]
    ]
    # df[SURVEY_TOUR_ID] = df['tour_id'].copy()
    df.to_csv(
        os.path.join(output_dir, "survey_joint_tour_participants.csv"), index=False
    )
    ...
    joint_tour_participants = pd.read_csv(
        os.path.join(output_dir, "survey_joint_tour_participants.csv")
    )
    ...
    tour.loc[
        (tour["origin"] == 38677)
        & (tour["destination"] == 20072)
        & (tour["tour_mode"] == "WALK_LOC"),
        "tour_mode",
    ] = "WALK_FRY"
    tour.loc[
        (tour["origin"] == 22883)
        & (tour["destination"] == 38711)
        & (tour["tour_mode"] == "WALK_LOC"),
        "tour_mode",
    ] = "WALK_FRY"
    tour.loc[
        (tour["origin"] == 37112)
        & (tour["destination"] == 12648)
        & (tour["tour_mode"] == "WALK_LOC"),
        "tour_mode",
    ] = "WALK_FRY"
    tour.loc[
        (tour["origin"] == 40711)
        & (tour["destination"] == 22228)
        & (tour["tour_mode"] == "WALK_LOC"),
        "tour_mode",
    ] = "WALK_FRY"
    ...
    person.loc[
        ...
        & (
            (person["school_zone_id"] == missing_school_zone)
    ...
    ].to_csv(os.path.join(output_dir, "person_and_household_id_mapping.csv"))
    person["person_id"] = person["person_id"].astype("int32")
    households["person_id"] = households["household_id"].astype("int32")
    trip[["trip_id", "trip_id", "trip_id_elmer"]].to_csv(
        os.path.join(output_dir, "trip_id_mapping.csv")
    )

    # Set new person and household ID on all other files
    trip.drop("household_id", inplace=True, axis=1)
    trip = trip.merge(
        person[["person_id", "person_id_original", "household_id"]],
        left_on="person_id",
    ...
    joint_tour_participants.drop(
        ["person_id_x", "person_id_original"], axis=1, inplace=True
    )
    ...
    tour = tour.merge(
        person[["person_id", "workplace_zone_id", "school_zone_id"]],
        on="person_id",
        how="left",
    )
    df = tour[tour["tour_type"] == "work"][
        ["destination", "tour_id", "workplace_zone_id"]
    ]

    df1 = df.merge(
        maz_shp, left_on="destination", right_on="maz_id", how="left"
    ).set_index("tour_id")
    df2 = df.merge(
        maz_shp, left_on="workplace_zone_id", right_on="maz_id", how="left"
    ).set_index("tour_id")
    ...
    for tour_id in change_destination:
        ...
        df = trip.loc[(trip["tour_id"] == tour_id) & (trip["outbound"] == True)].iloc[
            -1
        ]
        trip.loc[trip["trip_id"] == df["trip_id"], "destination"] = tour.loc[
            tour["tour_id"] == tour_id, "destination"
        ].values[0]
        # change first inbound origin
        df = trip.loc[(trip["tour_id"] == tour_id) & (trip["outbound"] == False)].iloc[
            0
        ]
        trip.loc[trip["trip_id"] == df["trip_id"], "origin"] = tour.loc[
            tour["tour_id"] == tour_id, "destination"
        ].values[0]

    # Adjust trip & tour destinations for school travel
    df = tour[tour["tour_type"] == "school"][
        ["destination", "school_zone_id", "tour_id"]
    ]
    df1 = df.merge(
        maz_shp, left_on="destination", right_on="maz_id", how="left"
    ).set_index("tour_id")
    df2 = df.merge(
        maz_shp, left_on="school_zone_id", right_on="maz_id", how="left"
    ).set_index("tour_id")
    dist_df = gpd.GeoSeries(df1.geometry).distance(gpd.GeoSeries(df2.geometry))
    change_destination = dist_df[dist_df <= 5280].index
    drop_tours = dist_df[dist_df > 5280].index
    ...
    joint_tour_participants.to_csv(
        os.path.join(output_dir, "final_survey_joint_tour_participants.csv"),
        index=False,
    )
    tour.to_csv(os.path.join(output_dir, "final_survey_tours.csv"), index=False)
    households.to_csv(
        os.path.join(output_dir, "final_survey_households.csv"), index=False
    )
