...
person_wt_col = (
    "hh_weight_2017_2019"  # no specific person weight available, use HH weight
)
...
hh["household_id_elmer"] = (
    hh["household_id_original"].apply(lambda x: x.split("_")[0]).astype("int64")
)
...
hh_elmer = pd.read_sql(
    sql="SELECT * FROM HHSurvey.v_households WHERE survey_year IN (2017, 2019)",
    con=engine,
)

hh = hh.merge(
    hh_elmer[["household_id", hh_wt_col]],
    left_on="household_id_elmer",
...
person["person_id_elmer"] = (
    person["person_id_original"].apply(lambda x: x.split("_")[0]).astype("int64")
)
...
# Join weights from Elmer
person_elmer = pd.read_sql(
    sql="SELECT * FROM HHSurvey.v_persons WHERE survey_year IN (2017, 2019)", con=engine
)

person = person.merge(
    person_elmer[["person_id", person_wt_col]],
    left_on="person_id_elmer",
    right_on="person_id",
)
...
trip_elmer = pd.read_sql(
    sql="SELECT * FROM HHSurvey.v_trips WHERE survey_year IN (2017, 2019)", con=engine
)
...
trip = trip.merge(
    trip_elmer[["trip_id", trip_wt_col]], left_on="trip_id_elmer", right_on="trip_id"
)
