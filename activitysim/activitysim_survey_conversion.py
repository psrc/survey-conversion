...
if geocode_trips:
    if read_survey_from_db:
        trip = pd.read_sql(
            sql="SELECT * FROM HHSurvey.v_trips WHERE survey_year IN (2017, 2019)",
            con=engine,
        )
    else:
...
if geocode_households:
    if read_survey_from_db:
        hh = pd.read_sql(
            sql="SELECT * FROM HHSurvey.v_households WHERE survey_year IN (2017, 2019)",
            con=engine,
        )
    else:
...
if geocode_persons:
    ...
    if read_survey_from_db:
        person = pd.read_sql(
            sql="SELECT * FROM HHSurvey.v_persons WHERE survey_year IN (2017, 2019)",
            con=engine,
        )
    else:
...
if convert_survey:
    ...
    # FIXME: use all representative survey days by assigning new household and person IDs
    ...
    # Create tours from from trip file that has already been partially processed above
    ...
    for personid in trip[person_id_col].unique():
        ...
        for day in person_df[day_col].unique():
            ...
            if (
                df.groupby(person_id_col).first()[opurp].values[0] != "Home"
            ) or df.groupby(person_id_col).last()[dpurp].values[0] != "Home":
                #    # Flag this set
                for i in df[trip_id].to_list():
            ...
            for tour_start_index in range(len(home_tours_start)):
                ...
                tour_dict[tour_id]["tlvorig"] = _df.iloc[0][
                    deptm
                ]  # Time leaving origin
                        ...
                        for subtour_start_value in subtour_index_start_values:
                            ...
                            for i in _df.index.values[next_row_index_start:]:
                                ...
                                if (
                                    ...
                                    tour_dict[tour_id][person_id_col] = subtour_df.iloc[
                                        0
                                    ][person_id_col]
                                    tour_dict[tour_id]["day"] = day
                                    ...
                                    if len(subtour_df) == 2:
                                        ...
                                        tour_dict[tour_id]["tripsh1"] = 1
                                        tour_dict[tour_id]["tripsh2"] = 1
                                        tour_dict[tour_id][tdadtyp] = subtour_df.iloc[
                                            0
                                        ][dadtyp]
                                        tour_dict[tour_id][toadtyp] = subtour_df.iloc[
                                            0
                                        ][oadtyp]
                                        tour_dict[tour_id]["tdtaz"] = subtour_df.iloc[
                                            0
                                        ][dtaz]
                                        tour_dict[tour_id][tdpcl] = subtour_df.iloc[0][
                                            dpcl
                                        ]
                                        tour_dict[tour_id]["tlvdest"] = subtour_df.iloc[
                                            -1
                                        ][deptm]
                                        tour_dict[tour_id]["tardest"] = subtour_df.iloc[
                                            0
                                        ][arrtm]
                                        tour_dict[tour_id][
                                            tour_mode
    ...
    for index, row in tour.iterrows():
        ...
        filter = (
            (tour.day == row.day)
            & (tour.tour_type == row.tour_type)
            & (tour.topcl == row.topcl)
            & (tour.tdpcl == row.tdpcl)
            & (tour.topcl == row.topcl)
            & (tour.tdpcl == row.tdpcl)
            & (tour.tour_mode == row.tour_mode)
    ...
    df = tour[
        ((tour["joint_tour"].isin(joint_tour_list)) & (~tour["joint_tour"].isnull()))
    ]
    ...
    # Create a new mapping for person ID
    person["person_id_original"] = person["person_id"].copy()
    person["person_id"] = range(1, len(person) + 1)
    # Store mapping for later reference
    person[["person_id", "person_id_original"]].to_csv(
        os.path.join(output_dir, "person_id_mapping.csv")
    )
    person["person_id"] = person["person_id"].astype("int32")

    # Set new person ID on all other files
    trip = trip.merge(
        person[["person_id", "person_id_original"]],
        left_on="person_id",
    ...
    tour["drop"] = 0
    tour.loc[
        (tour["parent_tour_id"] != 0) & ~(tour["parent_tour_id"].isin(tour.tour_id)),
        "drop",
    ] = 1
    logger.info(
    ...
    tour = tour.merge(
        hh[["household_id", "home_zone_id"]], on="household_id", how="left"
    )
    ...
    trip = trip.merge(person[["person_id", "pstudent"]], on="person_id", how="left")
    trip.loc[
